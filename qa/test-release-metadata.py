...
require(header.group(1) == EXPECTED_VERSION, "plugin Version header is not 2.1.2")
require(constant.group(1) == EXPECTED_VERSION, "JIULIU_CRYPTO_VERSION is not 2.1.2")
...
require(stable is not None and stable.group(1) == EXPECTED_VERSION, "Stable tag is not 2.1.2")
require(
    changelog is not None and changelog.group(1).strip() == EXPECTED_VERSION,
    "latest readme changelog entry is not 2.1.2",
)
...
for label, document in (("plugin README", plugin_readme), ("root README", root_readme)):
    require(EXPECTED_ZIP in document, f"{label} does not name the 2.1.2 ZIP")
...
require(
    ...
    "SHA256SUMS does not contain the canonical 2.1.2 dist artifact",
)
require(not sums.startswith("0" * 64), "SHA256SUMS still contains the pre-release placeholder")

print("PASS: plugin header, stable tag, changelog, docs, build metadata and checksum target are 2.1.2")
