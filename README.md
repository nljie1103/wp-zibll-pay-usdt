...
后台路线管理器将配置完整度与运行状态分开显示。已配置路线无论启用或停用都保留在同一列表，可即时启停或单独保存；尚未完成配置的路线按币种分组收纳，并提供搜索及独立的配置/运行状态筛选。路线详细配置父级默认收起，展开后才显示所有路线标题；每条路线只有一套真实配置表单，单路线保存只写入目标路线，停用不会清空地址、RPC、汇率或只读凭据。
...
2. 上传 `jiuliu-crypto-payment-2.1.3.zip`，安装并启用。
...
- 安装包：`dist/jiuliu-crypto-payment-2.1.3.zip`
- 校验文件：`SHA256SUMS`
- 自动化检查：`qa/`
