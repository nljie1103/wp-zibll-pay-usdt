...
后台路线管理器将已启用路线紧凑展示，把尚未启用的路线按币种分组收纳，并提供币种、网络、链 ID 搜索和状态/币种筛选。每条路线只有一套真实配置表单；地址、RPC、汇率和只读凭据在路线停用后仍会原样保存，合约及不可变链信息收纳在默认折叠的“高级与安全核验信息”中。
...
2. 上传 `jiuliu-crypto-payment-2.1.1.zip`，安装并启用。
...
- 安装包：`dist/jiuliu-crypto-payment-2.1.1.zip`
- 校验文件：`SHA256SUMS`
- 自动化检查：`qa/`
