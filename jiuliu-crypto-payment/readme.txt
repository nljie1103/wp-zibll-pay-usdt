...
* 三层后台路线管理器：已配置路线紧凑展示，配置状态与运行状态分离，并支持即时启停、单路线保存、搜索和筛选
...
* 将“已启用路线”重构为“已配置路线”，配置完整度与运行状态独立显示；已配置路线停用后保留全部地址、RPC、只读凭据、汇率和确认数。
* 新增受保护的单路线即时启停，以及“取消”“保存配置”“保存并启用此路线”“停用并保存”；仍复用原设置白名单、Nonce、权限和安全校验。
* 修复详细配置收起后标题消失、展开/收起语义错误、未保存状态不清楚及总网关缺少路线提示的问题。
* 将前台统一入口名称缩短为“数字货币支付”，修复移动端弹窗溢出、关闭按钮不可用和遮罩无法关闭；关闭界面不会取消订单或停止链上监听。
