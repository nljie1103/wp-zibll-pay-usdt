...
后台“币种与网络路线”将配置状态与运行状态分开：已配置路线即使停用也保留在主列表，可直接启停；配置不完整或尚未配置的路线收纳在添加区域。“路线详细配置”父级默认收起，展开后才显示全部路线标题。每条路线提供取消、保存配置、保存并启用或停用并保存，且只有一套真实表单；单路线保存只写入目标路线，不会连带保存其他路线或全局设置。停用不会丢失地址、RPC、汇率或只读凭据；无 JavaScript 时仍可通过原生 `details` 和“保存全部设置”操作。
...
1. 在 WordPress 后台上传并启用 `jiuliu-crypto-payment-2.1.3.zip`。
2. 打开“多链收款 → 设置”。
