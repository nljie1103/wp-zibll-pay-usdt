...
后台“币种与网络路线”采用三层管理界面：第一层紧凑列出已启用路线，第二层按币种收纳可添加路线并支持搜索/筛选，第三层保留每条路线唯一的完整配置。停用路线仍留在同一表单中，保存时不会丢失地址、RPC、汇率或只读凭据；无 JavaScript 时也可以直接展开原生 `details` 完成配置。
...
1. 在 WordPress 后台上传并启用 `jiuliu-crypto-payment-2.1.1.zip`。
2. 打开“多链收款 → 设置”。
