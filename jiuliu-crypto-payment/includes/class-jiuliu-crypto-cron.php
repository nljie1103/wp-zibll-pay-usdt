...
class JIULIU_CRYPTO_Cron
{
    const EVENT = 'jiuliu_crypto_scan_event';
    ...
    public static function add_schedule($schedules)
    {
        $schedules['jiuliu_every_minute'] = array(
            'interval' => 60,
            'display'  => __('每分钟（九流多链支付）', 'jiuliu-crypto-payment'),
        );
        return $schedules;
    }
    ...
    public function run()
    {
        ...
        if (get_transient('jiuliu_crypto_scan_lock')) {
        ...
        set_transient('jiuliu_crypto_scan_lock', 1, 55);
        try {
            if (false === $this->db->recover_stale_processing(5)) {
                throw new RuntimeException(__('无法恢复超时的结算处理中支付单。', 'jiuliu-crypto-payment'));
            }
            $stats = $this->invoices->scan_pending();
            if (!is_array($stats)) {
                throw new RuntimeException(__('链上扫描返回了无效结果。', 'jiuliu-crypto-payment'));
            }
            // Monitored expired/superseded/closed invoices release their exact
            // amount tails inside scan_pending(), and only after their complete
            // chain window is covered. Here we clean only terminal states that
            // are intentionally outside automatic monitoring.
            if (false === $this->db->release_terminal_active_keys($this->settings->get('late_grace_hours', 24))) {
                throw new RuntimeException(__('无法释放终态支付单的金额尾数。', 'jiuliu-crypto-payment'));
            }

            if (0 === ((int) gmdate('H')) && (int) gmdate('i') < 2) {
                if (false === $this->db->delete_old_logs($this->settings->get('log_retention_days', 90))) {
                    throw new RuntimeException(__('无法清理过期链上支付日志。', 'jiuliu-crypto-payment'));
                }
            }
    ...
    {
        register_rest_route('jiuliu-crypto/v1', '/cron', array(
    ...
    public function rest_permission(WP_REST_Request $request)
    {
        ...
        if (!$expected || !$provided || !hash_equals($expected, $provided)) {
            return new WP_Error('rest_forbidden', __('Cron 密钥无效。', 'jiuliu-crypto-payment'), array('status' => 403));
        }

        $allowlist = trim((string) $this->settings->get('cron_ip_allowlist'));
        if ($allowlist) {
            $allowed = preg_split('/[\s,]+/', $allowlist);
            $ip = JIULIU_CRYPTO_Util::client_ip();
            if (!$ip || !in_array($ip, $allowed, true)) {
                return new WP_Error('rest_ip_forbidden', __('当前 IP 不在 Cron 白名单中。', 'jiuliu-crypto-payment'), array('status' => 403));
            }
        }
