...
class JIULIU_CRYPTO_Zibll
{
    const SDK = 'jiuliu_crypto_v2';
    ...
    public function __construct(JIULIU_CRYPTO_Settings $settings, JIULIU_CRYPTO_DB $db, JIULIU_CRYPTO_Invoices $invoices, $routes = null)
    {
    ...
    public function register()
    {
        ...
        add_filter('plugin_action_links_' . plugin_basename(JIULIU_CRYPTO_FILE), array($this, 'plugin_action_links'));
    }

    public function add_payment_method($methods, $pay_type)
    {
        if (
            !(bool) $this->settings->get('enabled')
            || (bool) $this->settings->get('pause_monitoring')
    ...
    public function route_payment_sdk($pay_sdk, $order_data)
    {
        if (!empty($order_data['payment_method'])) {
            $route = $this->route_by_method($order_data['payment_method'], true);
            if ($route) {
                return self::SDK;
            }
        }
    ...
    public function initiate_payment($order_data)
    {
        ...
        if (
            ...
            || !$this->route_by_method((string) $payment['method'], false)
        ) {
            return array(
                'error' => 1,
                'msg'   => __('子比支付方式已经变化，请重新打开收银台。', 'jiuliu-crypto-payment'),
            );
        }
    ...
    public function lock_initiate_payment()
    {
        ...
        if (!$this->db->acquire_settlement_lock($payment_id, 10)) {
            wp_send_json_error(array(
                'code' => 'jiuliu_crypto_payment_busy',
                'msg'  => __('该订单正在确认链上到账，请稍后刷新，暂时不要切换支付方式。', 'jiuliu-crypto-payment'),
            ), 409);
            return;
        }
        ...
        if (!$this->is_plugin_method($current_method) && !$this->is_plugin_method($requested_method)) {
        ...
        if (is_wp_error($authorized)) {
            wp_send_json_error(array(
                'code' => 'jiuliu_crypto_payment_forbidden',
                'msg'  => $authorized->get_error_message(),
            ), 403);
        }
    }

    private function payment_routes($enabled_only = false)
    {
        if ($this->routes && is_callable(array($this->routes, 'all'))) {
            $routes = $this->routes->all($enabled_only);
    ...
    private function is_plugin_method($method)
    {
        return (bool) $this->route_by_method($method, false);
    }
    ...
    private function authorize_initiate($order_data)
    {
        if (!is_array($order_data) || empty($order_data['payment_id'])) {
            return new WP_Error('zibll_payment_forbidden', __('支付单校验失败，请从原订单页面重新打开收银台。', 'jiuliu-crypto-payment'));
        }
        ...
        foreach ($orders as $order) {
            ...
            if (!$order_id || !isset($order['status']) || '0' !== (string) $order['status']) {
                return new WP_Error('zibll_payment_forbidden', __('支付单已经失效，请从原订单页面重新打开收银台。', 'jiuliu-crypto-payment'));
            }

            if ($owner_id) {
                if (!$current_user_id || $current_user_id !== $owner_id) {
                    return new WP_Error('zibll_payment_forbidden', __('无权发起此支付单，请从原订单页面重新打开收银台。', 'jiuliu-crypto-payment'));
                }
                continue;
            }
    ...
    private function get_order_proof_html($order, $panel)
    {
        ...
        if (
            ...
            || !$this->is_plugin_method((string) $order['pay_type'])
            || !is_callable(array('ZibPay', 'get_meta'))
        ) {
            return '';
        }

        $proof = ZibPay::get_meta(absint($order['id']), 'jiuliu_crypto_payment', array());
        if (!is_array($proof)) {
            return '';
        }

        $txid = !empty($proof['txid']) ? JIULIU_CRYPTO_Util::normalize_txid($proof['txid']) : '';
        ...
        $details = '<span class="jiuliu-crypto-order-proof-network">' . esc_html($asset_symbol) . ' · ' . esc_html($network) . '</span>';
        if ($amount) {
            $details .= '<span>' . esc_html($amount) . ' ' . esc_html($asset_symbol) . '</span>';
        }
        if ($invoice_no) {
            $details .= '<span>' . esc_html__('凭证', 'jiuliu-crypto-payment') . ' ' . esc_html($invoice_no) . '</span>';
        }
        $details .= '<span title="' . esc_attr($txid) . '">' . esc_html__('交易', 'jiuliu-crypto-payment') . ' <code>' . $short_txid . '</code></span>';

        if ($panel) {
            return '<div class="jiuliu-crypto-order-proof-panel zib-widget">'
                . '<div class="jiuliu-crypto-order-proof-title">' . esc_html__('链上支付凭证', 'jiuliu-crypto-payment') . '</div>'
                . '<div class="jiuliu-crypto-order-proof">' . $details . '</div>'
                . '</div>';
        }

        return '<span class="jiuliu-crypto-order-proof jiuliu-crypto-order-proof-footer">' . $details . '</span>';
    }
    ...
    public function normalize_order_before_create($order_data)
    {
        ...
        $contains_plugin_detail = false;
        if (is_array($pay_detail)) {
            foreach (array_keys($pay_detail) as $detail_method) {
                if ($this->is_plugin_method($detail_method)) {
                    $contains_plugin_detail = true;
                    break;
                }
            }
        }
        if ($contains_plugin_detail) {
    ...
    {
        if (!is_object($order) || empty($order->id) || empty($order->pay_type) || !$this->is_plugin_method($order->pay_type)) {
            return;
        }

        global $wpdb;
        $original = isset($order->pay_detail) ? maybe_unserialize($order->pay_detail) : array();
        $route = $this->route_by_method($order->pay_type, false);
        $chain_scope = $route ? strtolower((string) $route['adapter']) . '|' . strtolower((string) $route['chain_id']) : '';
        // zibpay_order.pay_num is varchar(50) in Zibll 9.0, while an EVM/TRON
        // transaction hash is 64 hexadecimal characters. Zibll stores the full
        // value on the parent payment row, so never trust the truncated child
        // value when attaching immutable chain proof.
        $parent = !empty($order->payment_id) && class_exists('ZibPay') && is_callable(array('ZibPay', 'get_payment'))
            ? ZibPay::get_payment(absint($order->payment_id))
            : false;
        $txid = is_array($parent) && !empty($parent['pay_num'])
            ? JIULIU_CRYPTO_Util::normalize_txid($parent['pay_num'])
            : '';
        $invoice = $txid ? $this->db->get_by_txid($txid, $chain_scope) : null;

        if (class_exists('ZibPay') && is_callable(array('ZibPay', 'update_meta'))) {
            ZibPay::update_meta($order->id, 'jiuliu_crypto_payment', array(
                'invoice_no'  => $invoice ? $invoice->invoice_no : '',
                'network'     => $invoice && !empty($invoice->network) ? $invoice->network : ($route ? $route['network'] : ''),
                'asset_symbol'=> $invoice && !empty($invoice->asset_symbol) ? $invoice->asset_symbol : ($route ? $route['asset_symbol'] : 'USDT'),
                'asset_amount'=> $invoice ? $invoice->actual_amount : '',
    ...
    public function mark_success_hooks_completed($order)
    {
        if (
            ...
            || empty($order->id)
            || empty($order->pay_type)
            || !$this->is_plugin_method($order->pay_type)
            || !class_exists('ZibPay')
        ...
        ZibPay::update_meta($order->id, 'jiuliu_crypto_success_hooks_completed', JIULIU_CRYPTO_Util::utc_now_mysql());
    }
    ...
    public function preflight_check_pay()
    {
        ...
        if (!empty($invoice->last_checked_at)) {
            $last_checked = JIULIU_CRYPTO_Util::utc_timestamp_from_mysql($invoice->last_checked_at);
            if ($last_checked > time() - 8) {
                return;
            }
        }

        if (!$this->consume_preflight_scan_budget()) {
            return;
        }
    ...
    private function consume_preflight_scan_budget()
    {
        ...
        $global_limit = (int) apply_filters('jiuliu_crypto_preflight_global_limit', 120);
        $ip_limit = (int) apply_filters('jiuliu_crypto_preflight_ip_limit', 20);
        $global_limit = max(1, min(1000, $global_limit));
        $ip_limit = max(1, min(120, $ip_limit));

        $ip = (string) JIULIU_CRYPTO_Util::client_ip();
        $ip_hash = substr(hash('sha256', $ip ? $ip : 'unknown'), 0, 20);
        $global_key = 'jiuliu_crypto_preflight_global_' . $window;
        $ip_key = 'jiuliu_crypto_preflight_ip_' . $window . '_' . $ip_hash;
    ...
    {
        if (!(bool) $this->settings->get('enabled') || (bool) $this->settings->get('pause_monitoring') || is_admin()) {
            return;
        }

        wp_enqueue_style(
            'jiuliu-crypto-payment',
            JIULIU_CRYPTO_URL . 'assets/css/frontend.css',
            array(),
            JIULIU_CRYPTO_VERSION
        );
        wp_enqueue_script(
            'jiuliu-crypto-payment',
            JIULIU_CRYPTO_URL . 'assets/js/frontend.js',
            array('jquery'),
            JIULIU_CRYPTO_VERSION,
            true
        );
        wp_localize_script('jiuliu-crypto-payment', 'jiuliuCrypto', array(
            ...
            'copyFail' => __('复制失败，请手动复制', 'jiuliu-crypto-payment'),
    ...
    public function compatibility_notice()
    {
        ...
        if ($screen && false === strpos($screen->id, 'jiuliu-crypto')) {
            return;
        }

        if (!$this->is_zibll_template()) {
            echo '<div class="notice notice-warning"><p>'
                . esc_html__('九流网络多链支付已安装，但当前父主题不是 Zibll。网关不会在前台启用。', 'jiuliu-crypto-payment')
                . '</p></div>';
            return;
        }
        ...
        if (!$this->has_required_api()) {
            echo '<div class="notice notice-error"><p>'
                . esc_html__('当前 Zibll 缺少 V9 支付扩展接口（ZibPay::payment_order 或支付函数）。请确认主题为完整的 Zibll 9.0。', 'jiuliu-crypto-payment')
                . '</p></div>';
            return;
        }
    ...
    public function plugin_action_links($links)
    {
        array_unshift(
            $links,
            '<a href="' . esc_url(admin_url('admin.php?page=jiuliu-crypto')) . '">' . esc_html__('设置', 'jiuliu-crypto-payment') . '</a>'
        );
        return $links;
    }
